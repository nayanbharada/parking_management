...
class SlotListView(CustomActiveLoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Slot.objects.filter(premise_slot__premise_user=self.request.user).active()
        return queryset
...
class AllSlotView(ListView):
    ...
    def get_queryset(self):
        ...
        if start_date and end_date and vehicle_type:
            ...
            booking_slots = BookParking.objects.filter(start_time__lte=end_date,
                                                      end_time__gte=start_date).values_list('slot_book', flat=True)
...
class BookParkingSlotView(View):
    def post(self, request, *args, **kwargs):
        ...
        obj = BookParking.objects.create(slot_book_id=slot_id,
                                   user=request.user,
